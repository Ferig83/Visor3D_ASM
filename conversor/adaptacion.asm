; Esto estaría en el CreateFile. Lo que tenemos que hacer es poner todo lo que puse en paint en WM_CREATE
; y pintar sólo cuando dice WM_PAINT.

; Lo que necesito hacer es leer el archivo segun parámetro en el command line, y listo, volcarlo a
; una estructura de tamaño indefinido :|

; Hay que usar memoria dinamica acá...
...
;
;call GetProcessHeap
; el handle queda en rax

;mov rcx, rax  ; aca va el handle
;mov rdx, 8    ; esto es HEAP_ZERO_MEMORY (te la setea a 0, re copado)
;mov r8, 6     ; cantidad de bytes a ubicar
;call HeapAlloc

;creo que sigue el puntero en rax...(devuelve un DECLSPEC_ALLOCATOR o algo asi)
;
;mov [rax], alguna mierda de 6 bytes  ; para que se vea que se puede escribir
;
;y despues no se si hay que borrar 
;mov rcx, rax  ; el handle
;call HeapDestroy

; hay un HeapFree pero dice que podes llamar a HeapDestroy (no
; se si esto implica que se llame a HeapFree primero).
